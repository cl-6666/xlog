...
public class XFilePrinter implements XLogPrinter {
    ...
    private static XFilePrinter instance;
     ...
     */
    public static synchronized XFilePrinter getInstance(String logPath, long retentionTime) {
        if (instance == null) {
            instance = new XFilePrinter(logPath, retentionTime);
        }
        return instance;
    }
    ...
    public void print(@NonNull XLogConfig config, int level, String tag, @NonNull String printString) {
        ...
        worker.put(new XLogMo(timeMillis, level, tag, printString));
    }

    private void doPrint(XLogMo logMo) {
    ...
    private class PrintWorker implements Runnable {
        ...
        private BlockingQueue<XLogMo> logs = new LinkedBlockingQueue<>();
